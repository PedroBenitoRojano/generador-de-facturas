...
export function generateInvoiceHTML(invoiceData, globalData) {
    ...
    const iva = items.reduce((acc, item) => {
        if (item.taxType === 'IVA') return acc + (item.quantity * item.price * (item.taxValue / 100))
        return acc
    }, 0)
  ...
  `).join('')
    ...
    // We Use the provided template and replace the relevant parts
    return `
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <style>
        ...
        .invoice-box { max-width: 800px; background: #fff; margin: 0 auto; padding: 40px; }
